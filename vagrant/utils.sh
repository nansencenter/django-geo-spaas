sudo apt-get update -qq
sudo apt-get install -q -y python-software-properties

sudo add-apt-repository -y ppa:ubuntugis/ubuntugis-unstable
sudo add-apt-repository -y ppa:marlam/gta

sudo apt-get update -qq

sudo apt-get install subversion -y
sudo apt-get install python-numpy -y
sudo apt-get install python-dev -y
sudo apt-get install postgis -y
sudo apt-get install postgresql-server-dev-9.1 -y
sudo apt-get install postgresql-9.1-postgis -y
sudo apt-get install postgresql-9.1-postgis-scripts -y
sudo apt-get install libmysqlclient-dev -y
sudo apt-get install libpq-dev -y
sudo apt-get install libpng12-dev -y
sudo apt-get install libjpeg-dev -y
sudo apt-get install libgif-dev -y
sudo apt-get install liblzma-dev -y
sudo apt-get install libgeos-dev -y
sudo apt-get install libcurl4-gnutls-dev -y
sudo apt-get install libproj-dev -y
sudo apt-get install libxml2-dev -y
sudo apt-get install libexpat-dev -y
sudo apt-get install libxerces-c-dev -y
sudo apt-get install libnetcdf-dev -y
sudo apt-get install netcdf-bin -y
sudo apt-get install libpoppler-dev -y
sudo apt-get install libspatialite-dev -y
sudo apt-get install gpsbabel -y
sudo apt-get install swig -y
sudo apt-get install libhdf4-alt-dev -y
sudo apt-get install libhdf5-serial-dev -y
sudo apt-get install libpodofo-dev -y
sudo apt-get install poppler-utils -y
sudo apt-get install libfreexl-dev -y
sudo apt-get install unixodbc-dev -y
sudo apt-get install libwebp-dev -y
sudo apt-get install openjdk-7-jdk -y
sudo apt-get install libepsilon-dev -y
sudo apt-get install libgta-dev -y
sudo apt-get install liblcms2-2 -y
sudo apt-get install libpcre3-dev -y
sudo apt-get install libjasper-dev -y
sudo apt-get install libarmadillo-dev -y
sudo apt-get install make -y
sudo apt-get install g++ -y
sudo apt-get install autoconf -y
sudo apt-get install cmake -y
sudo apt-get install bison -y
sudo apt-get install flex -y
sudo apt-get install doxygen -y
sudo apt-get install vim -y
sudo apt-get install libdap-dev -y
sudo apt-get install git -y
sudo apt-get install spatialite-bin -y
sudo apt-get install python-gtk2 -y
sudo apt-get install python-gtk2-dev  -y
sudo apt-get install x11-apps -y
sudo apt-get install tk tk-dev -y
sudo apt-get install libcanberra-gtk-module

# add history lookup with pg-up, pg-dwn
echo '# alternate mappings for "page up" and "page down" to search the history' >> /home/vagrant/.inputrc
echo '"\e[5~": history-search-backward' >> /home/vagrant/.inputrc
echo '"\e[6~": history-search-forward' >> /home/vagrant/.inputrc
